...
- **BETA — market posture (the tide).** When BTC is in a clean trend, the lane below shows what the alpha gate *correctly* refuses to call edge but a trader still wants to see: the **top-3 structure-timed with-tide candidates** whose setup×regime historically **makes money** (exp>0 on adequate effective sample) but adds **no timing alpha beyond the trend** — each with the exact shortfall ("edge +0.01R ≤ floor", "excess not significant"), plus the **near-misses** closest to proving alpha, cache-wide. **BETA ≠ ALPHA:** profits ride *with* the trend; `stage` labels them *unproven — discretionary*; `roar` never strikes them; size conservatively — the tide can turn without notice. By default `scan` now deep-scans **all survivors** from the top-100 screen (not a fixed 8), concurrently — wider net, but a cold/full scan takes ~1–2 min. `--top N` caps it if you want it faster.
