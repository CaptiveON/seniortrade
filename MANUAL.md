...
> **DERIVATIVES FEATURES (A1/A3):** every scan now (a) tops up each pool coin's **full funding history** and tags every backtested trade with a PIT-honest funding bucket (`fund`: 30-day z-score → ext−…ext+) — backtest costs use the **actual settled funding** over each hold, not a flat assumption; (b) **collects the 30-day OI + long/short-ratio window** into `data/derivs/` (append-dedupe — the local history grows past Binance's retention forever) and tags an `oi` bucket (expand/flat/contract vs 24h). Both are ordinary context features: they change nothing until a cell clears money + Bonferroni + OOS; `features` tracks their measured importance.
...
**Phase C — the Risk Cockpit view:** the L0–L3 profile cards with a live **old→new diff modal** behind the same typed-CONFIRM rail (appetite changes deserve friction); the applied posture is echoed, shown in the topbar, and journaled onto every staged trade. **Strictness is deliberately read-only in the UI** — the truth bar has no button (set `EDGE_SIG_Z` in `.env`, and the cockpit displays the red measured-cost warning when you do). The 13 honesty-locked gates are listed on-screen. Profile scope = the cockpit session (server process); set `RISK_PROFILE` in `.env` to persist.
