...
2. **`git init` + pin deps + CI running the suite** (a day of work; converts "honest" into "provably honest").
3. Cross-coin correlation (Finding 2): footer caveat immediately; then cluster-robust SEs or effective-n deflation.
4. Candle sanitation screens in `fetch_ohlcv` (gaps, zero-volume, outliers → warn/drop) (Finding 6).
5. Longer history (pagination supports it) + time-decay ON as the default research posture (Finding 7).
6. Block-bootstrap variant for MC drawdowns; document i.i.d. assumptions in MANUAL (Finding 4); label universe-selection bias in footers (Finding 3).
