...
7. **Tide/beta lane + near-miss transparency** — ✅ **DONE 2026-07-03** (owner chose: both lanes always, deliberately labelled; top-3 structure-timed candidates). The board now has TWO LANES: **ALPHA** (proven timing edge — gates untouched) and **BETA** (market posture) shown whenever BTC trends: the null's own measurement of the tide, the top-3 live with-tide candidates whose setup×regime is historically +EV on adequate effective n but failed only the alpha gate (each with its exact shortfall), and the cache-wide **near-misses**. Guidance printed: BETA ≠ ALPHA, `stage` labels discretionary, `roar` never strikes beta. Pure additions (`beta_candidate`, `near_misses`); alpha path byte-identical. Tests +3 → 286.
   **HARDENED to PROVEN-BETA (2026-07-03, owner: "make it as competitive as alpha"):** the owner's challenge was proven correct — the original criterion (exp>0, no significance) fired on **23% of pure-noise cells** (alpha: 0%), two of three live candidates had P(exp>0) ≤ 88% / fold 0.00, and the "tide pays" premise was unsupported (all down-regime nulls negative). Rebuilt with two alpha-grade gates: **(1) tide_drift** — the combined with-tide null (n-weighted across setups) must clear sig_z (the tide itself proven +EV); **(2) vehicles** — cell significantly +EV at sig_z, OOS fold-consistent ≥0.5, adequate n_eff, ranked by conservative bound. Re-validated on the same noise: **0 qualifying cells, drift not proven — fluke-proof like alpha.** Live board now honestly refuses ("DRIFT NOT PROVEN, −0.132R over 27,590 shadows — no beta trades offered") and labels near-misses as hypotheses. Tests → 292.

**PHASE 3 — parked (owner-sequenced):**
8. Risk-profile system — ✅ **DONE 2026-07-10.** `RISK_PROFILES` L0–L3 registry (appetite+selectivity only), `_HONESTY_LOCKED` enforced at import + tested (no profile can touch sig_z/null/effective-n/sample floors/live rails); precedence defaults→tier→profile→env (env wins, proven); posture echoed every run; strictness axis separate with measured fluke cost printed red (1.28→~12%, 1.00→~16%); staged trades journaled with their profile. Live-proven: L2 flows 3% → pipeline → $7.50 on a weak wild edge (below L0-flat — intelligent appetite). Tests +7 → 299.
